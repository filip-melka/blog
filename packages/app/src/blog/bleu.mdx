...
isDraft: false
pubDate: 2025-10-31T22:03:21.553Z
---
...
> BLEU is an algorithm for evaluating the quality of text that has been machine-translated from one natural language to another.

Developed by researchers at IBM in 2001 (Papineni et al., 2002), BLEU was among the first metrics to show a strong correlation with human judgment. Despite its known limitations, it remains widely used today because it’s **simple**, **fast**, and **language-agnostic**.
...
**Note:** Although BLEU is typically computed across an entire corpus of translations (to smooth out sentence-level variability), we’ll use sentence-level examples for simplicity.

Before diving into how BLEU works, we need to understand two key ideas: **n-grams** and **precision**.

## What Are N-grams?

If you’ve done any NLP (Natural Language Processing) before, you’ve likely come across this term. In short:
...
To compute precision for any _n-gram_:

```math
Precision = \frac{\text{Number of correctly predicted n-grams}}{\text{Total number of predicted n-grams}}
```

Let's say we want to calculate the precision of 1-grams:

- **Reference:** "the bird flies over the water"
- **Prediction:** "bird flies across the water"

Four of the five _predicted_ words (1-grams) also appear in the _reference_:
...
1. Each predicted _n-gram_ is compared against _all_ references. If it appears in any reference, it’s counted as correct.
2. Each _n-gram_’s count is _clipped_ — it can’t appear more times in the prediction than it does in the references. This prevents the repetition problem.
...
- $r$ = length of the reference sentence (number of words in the _reference_)

The brevity penalty is never greater than 1 and becomes smaller when the _prediction_ is shorter than the _reference_.
...
Also keep in mind: a score of **0.6–0.7** is considered the best you can achieve. A score close to 1 is unrealistic and should raise a flag.
...
In our earlier example, we computed the geometric mean of 1-gram and 2-gram precisions by taking the square root of their product. This can also be written as:

```math
Geometric\ Mean\ Precision = (p_{1})^{\frac{1}{2}} \cdot (p_{2})^{\frac{1}{2}}
```

The exponents ($\frac{1}{2}$) act as weights. In this case both weights are equal. To give us more control, BLEU actually uses a **weighted geometric mean**, allowing us to emphasize shorter or longer _n-grams_.

For example, to give more weight to _bigrams_ (2-grams):

```math
Weighted\ Geometric\ Mean\ Precision = (p_{1})^{\frac{1}{3}} \cdot (p_{2})^{\frac{2}{3}}
```

You can choose any weights you want, as long as the sum of all weights equals 1.

By default, BLEU uses **up to 4-grams**, and we decided to use equal weights:
...
`method1` adds 1 to both the numerator and denominator for any _n-gram_ with zero matches, avoiding unfairly low scores for short or nearly correct translations.

![method1](./images/bleu/method1.png 'method1')
