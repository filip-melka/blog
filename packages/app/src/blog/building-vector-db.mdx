...
- **Custom-trained embeddings** – You can also train your own embedding model on a dataset of your choice, which is a great if you need domain-specific vocabulary (e.g., medicine, law).
...
Now we have a vector for each document. In real systems, embeddings are usually stored in a vector database, while the raw text is stored separately:
